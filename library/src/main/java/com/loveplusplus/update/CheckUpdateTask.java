...
class CheckUpdateTask extends AsyncTask<Void, Void, String> {
    ...
    private void parseJson(String result) {
        try {
            ...
            Double apkName = obj.getDouble(Constants.APK_VERSION_CODE_STRING);
            boolean isForceUpdate = obj.getBoolean(Constants.APK_FORCE_UPDATE);
            int versionCode = AppUtils.getVersionCode(mContext);
            String versionName=AppUtils.getVersionName(mContext);
            if(apkCode > versionCode){
                isForceUpdate=true;
            }
            if (apkCode > versionCode || apkName >  Double.parseDouble(versionName)) {
                if (mType == Constants.TYPE_NOTIFICATION) {
                    showNotification(mContext, updateMessage, apkUrl);
                } else if (mType == Constants.TYPE_DIALOG) {
                    showDialog(mContext, updateMessage, apkUrl,isForceUpdate);
                }
     ...
     * Show dialog
     */
    private void showDialog(Context context, String content, String apkUrl,boolean forceUpdate) {
